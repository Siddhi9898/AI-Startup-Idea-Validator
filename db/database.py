...
-- direct reset). Only a HASH of the token is stored, same principle
-- as password hashing - the raw token only ever exists in the email
-- link itself, never in the database, so a DB leak alone can't be
...
def update_password(email: str, new_password_hash: str) -> bool:
    """Overwrites a user's password hash (called only after a reset
    token has been verified - see tools/auth.py:confirm_password_reset).
    Returns True only if a matching user row was actually updated."""
    try:
        conn = _get_connection()
        try:
            with conn.cursor() as cur:
...
def create_password_reset_token(user_id: int, token_hash: str, expires_at) -> Optional[int]:
    """Stores a HASHED reset token (the caller hashes it - this
    module never sees the raw token). Returns the new row id, or
    None on failure."""
    try:
        conn = _get_connection()
        try:
        ...
        logger.warning("Could not create password reset token: %s", e)
        return None


def get_valid_reset_token(token_hash: str) -> Optional[dict]:
    """Returns {"id", "user_id", "email"} for a token hash that
    exists, hasn't expired, and hasn't already been used - or None
    if any of those checks fail (invalid/expired/reused token)."""
    try:
        conn = _get_connection()
        try:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        ...
        logger.warning("Could not validate reset token: %s", e)
        return None


def mark_reset_token_used(token_id: int) -> bool:
    try:
        conn = _get_connection()
        try:
            with conn.cursor() as cur:
        ...
        logger.warning("Could not mark reset token used: %s", e)
        return False
