...
# ---------------------------------------------------------------------------
# ---------------------------------------------------------------------------
# Password reset landing: the founder arrives here via the emailed
# link (?reset_token=...). Shown regardless of login state, above the
# normal Account section, and takes over the sidebar until resolved.
# ---------------------------------------------------------------------------
...
with st.sidebar:
    if _reset_token:
        ...
        st.caption("You followed a password reset link. Enter a new password below.")
        with st.form("confirm_reset_form"):
            new_pw = st.text_input("New password", type="password", help="At least 6 characters.")
            confirm_pw = st.text_input("Confirm new password", type="password")
            confirm_submitted = st.form_submit_button("Set New Password")
        if confirm_submitted:
            if new_pw != confirm_pw:
                st.error("Passwords don't match.")
            else:
                res = auth.confirm_password_reset(_reset_token, new_pw)
                if res["success"]:
                    st.success(res["message"] + " Removing the reset link from this page now.")
    ...
    if st.session_state["user"] is None:
        ...
        if auth_mode == "Login":
            ...
            reset_method = st.radio(
                "reset_method", ["Email Link", "Security Question"],
                horizontal=True, label_visibility="collapsed", key="reset_method_radio",
            )
            if reset_method == "Email Link":
                st.caption("Enter your account email - we'll send a password reset link to it, valid for 30 minutes.")
                with st.form("forgot_password_form"):
                    fp_email = st.text_input("Your account email")
                    fp_submitted = st.form_submit_button("Send Reset Link")
                if fp_submitted:
                    res = auth.request_password_reset(fp_email)
                    if res["success"]:
                        st.success(res["message"])
                    else:
                        st.error(res["message"])
            else:  # Security Question - works instantly, no SMTP needed
