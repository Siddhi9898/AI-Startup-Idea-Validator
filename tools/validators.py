...
def filter_relevant_results_with_fallback(
    extracted: dict,
    search_results: list,
    ...
    """
    Same deterministic relevance scoring as filter_relevant_results,
    but guarantees the agent isn't left with 0-1 results just because
    a handful of otherwise-decent hits scored a little under the
    strict threshold (this is what was starving every downstream
    agent of context - "less input on every agent").

    Behavior:
    1. Score every result exactly like filter_relevant_results.
    2. Keep everything that clears min_relevance (the strict, honest
       set) - order and scores unchanged.
    3. If that set has fewer than min_results items, backfill from
       the next-highest-scoring remaining results (those that scored
       above 0, just not above the strict threshold) until we reach
       min_results or run out of candidates. Backfilled items are
       tagged with below_threshold=True so callers/agents can still
       be honest about data quality if they choose to.
    ...
    strict = [r for r in scored if r["relevance"] >= min_relevance]
    if len(strict) >= min_results:
        return strict

    below = [r for r in scored if r["relevance"] < min_relevance]
    for r in below:
        if len(strict) >= min_results:
            break
